...
model_indices = zeros(size(x_orb(:,1)));
indexes = [1,2,3,4,5]
for p = 1:length(indexes)
 if strcmp(MODEL, '@etbp')
    p
    vector_field = zeros(3, length(x_orb(:,1)),length(x_orb(:,1)));
    index = indexes(p);
    t_HFEM = t_orb(index)/n_rtbp;
    META = 'kernels_to_load.tm'; %initialize required kernels
    cspice_furnsh(META); %furnish kernels
...
% Define a state vector for each grid point (considering only 2D for simplicity)
    for i = 1:1:2802
        i
            ...
            Z_rtbp(i) =  abs(norm(f1_val(4:6))- norm(f3_val(4:6))); %Difference of magnitudes of the acceleration vectors HFEM - RTBP
            Z_etbp(i) =  abs(norm(f1_val(4:6))- norm(f2_val(4:6)));%Difference of magnitudes of the acceleration vectors HFEM - ETBP
...
for i = 1:size(Z_rtbp, 1)
    ...
    if isnan(Z(i))
        fprintf('found a NaN at coordinates: %d, %d', X1(i),X2(i))
        Z(i) = 0;
    end
