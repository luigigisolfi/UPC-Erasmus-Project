...
[inertial_pos_spacecraft, inertial_vel_spacecraft, inertial_acc_spacecraft] = go_inertial(rs_rp, vs_vp, as_ap, oas_oap, SEb_pos, SEb_vel, SEb_acc, rtbp_pos, rtbp_vel, rtbp_acc, n_rtbp);
%[pos_spacecraft,vel_spacecraft, acc_spacecraft] = go_synodic(rs_rp, vs_vp, as_ap, oas_oap, SEb_pos, SEb_vel, SEb_acc, inertial_pos_spacecraft, inertial_vel_spacecraft, inertial_acc_spacecraft, n_rtbp);
...
for i = 1:N-1
    ...
    xiv=zeros(1,36);
    for p= 0:5 , xiv(7*p + 1)=1.e0; end  % variational matrix initalized with Id.
    fprintf('Integrating Variational Vector Field...\n')
    %[t_, phi_Q_tot] = ode45(@new_full_force, [t_sampled_inertial(i), t_sampled_inertial(i+1)], Q0(:,i));
    [t_, phi_Q_tot] = ode45(@new_full_force_var, [t_sampled_inertial(i), t_sampled_inertial(i+1)], [Q0(:,i);xiv.']);
    phi_Q = phi_Q_tot(:,1:6);
    ...
    plot(phi_Q(:,1), phi_Q(:,2), 'Color', 'Black', 'LineWidth',1)
    t_list_ = [t_list_; t_];
    F = phi_Q(end,:).' - Q0(:,i+1)
    F_list = [F_list; F];
